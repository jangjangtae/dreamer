...
class RetentionBandController:
  """
  Baseline-retention-based controller.

  baseline_score = baseline clean model performance (e.g., 11.8)
  green_ratio    = almost preserved zone
  yellow_ratio   = acceptable trade-off zone
  red zone       = competence degraded zone

  zone:
    green  : clean performance is well preserved
    yellow : acceptable loss, caution
    red    : competence degraded, protect gameplay
  """
  ...
  def __init__(
      ...
      min_beta_cov=0.01,
      max_beta_cov=0.20,
      lambda_task_up_yellow=0.03,
      lambda_task_up_red=0.08,
      lambda_task_decay=0.995,
      lambda_rep_lr=0.02,
      w_bug_up=0.01,
      w_bug_down_yellow=0.03,
      w_bug_down_red=0.12,
      beta_up=0.002,
      beta_down_yellow=0.02,
      beta_down_red=0.08,
      max_lambda_task=6.0,
      max_lambda_rep=3.0,
  ):
    ...
    self.lambda_task_up_yellow = float(lambda_task_up_yellow)
    ...
    self.w_bug_up = float(w_bug_up)
    self.w_bug_down_yellow = float(w_bug_down_yellow)
    self.w_bug_down_red = float(w_bug_down_red)

    self.beta_up = float(beta_up)
    self.beta_down_yellow = float(beta_down_yellow)
  ...
  def update(self, clean_score_mean=None, repeat_mean=None):
    ...
    # Task / exploration control by retention band
    if zone == 'green':
      self.lambda_task = float(np.clip(
          self.lambda_task * self.lambda_task_decay,
          0.5, self.max_lambda_task))
      self.w_bug = float(np.clip(
          self.w_bug + self.w_bug_up,
          self.min_w_bug, self.max_w_bug))
      self.beta_cov = float(np.clip(
          self.beta_cov + self.beta_up,
          self.min_beta_cov, self.max_beta_cov))

    elif zone == 'yellow':
      gap = self.green_floor - float(clean_score_mean)
      self.lambda_task = float(np.clip(
          self.lambda_task + self.lambda_task_up_yellow * max(gap, 0.0),
          0.5, self.max_lambda_task))
      self.w_bug = float(np.clip(
          self.w_bug - self.w_bug_down_yellow,
          self.min_w_bug, self.max_w_bug))
      self.beta_cov = float(np.clip(
          self.beta_cov - self.beta_down_yellow,
          self.min_beta_cov, self.max_beta_cov))
...
def tester_train(make_agent, make_replay, make_env, make_stream, make_logger, args):
  ...
  # --------------------------------------------------
  # Baseline-retention controller params
  # --------------------------------------------------
  baseline_score = float(os.getenv('TESTER_BASELINE_SCORE', '11.8'))
  green_ratio = float(os.getenv('TESTER_GREEN_RATIO', '0.90'))
  yellow_ratio = float(os.getenv('TESTER_YELLOW_RATIO', '0.75'))
  ...
  init_w_bug = float(os.getenv('TESTER_INIT_W_BUG', '0.8'))
  init_beta_cov = float(os.getenv('TESTER_INIT_BETA_COV', '0.12'))

  min_w_bug = float(os.getenv('TESTER_MIN_W_BUG', '0.25'))
  max_w_bug = float(os.getenv('TESTER_MAX_W_BUG', '1.40'))
  min_beta_cov = float(os.getenv('TESTER_MIN_BETA_COV', '0.01'))
  max_beta_cov = float(os.getenv('TESTER_MAX_BETA_COV', '0.20'))

  lambda_task_up_yellow = float(os.getenv('TESTER_LAMBDA_TASK_UP_YELLOW', '0.03'))
  lambda_task_up_red = float(os.getenv('TESTER_LAMBDA_TASK_UP_RED', '0.08'))
  lambda_task_decay = float(os.getenv('TESTER_LAMBDA_TASK_DECAY', '0.995'))
  lambda_rep_lr = float(os.getenv('TESTER_LAMBDA_REP_LR', '0.02'))

  w_bug_up = float(os.getenv('TESTER_W_BUG_UP', '0.01'))
  w_bug_down_yellow = float(os.getenv('TESTER_W_BUG_DOWN_YELLOW', '0.03'))
  w_bug_down_red = float(os.getenv('TESTER_W_BUG_DOWN_RED', '0.12'))

  beta_up = float(os.getenv('TESTER_BETA_COV_UP', '0.002'))
  beta_down_yellow = float(os.getenv('TESTER_BETA_COV_DOWN_YELLOW', '0.02'))
  beta_down_red = float(os.getenv('TESTER_BETA_COV_DOWN_RED', '0.08'))
  ...
  print('Retention band controller settings:')
  print(dict(
      baseline_score=baseline_score,
      green_ratio=green_ratio,
      yellow_ratio=yellow_ratio,
  ...
  def shape_reward(tran, worker):
    ...
    if tran['is_first']:
      ...
      if fault_episode:
        ...
        # Zone-based additional damping
        if controller.zone == 'red':
          exploration_term *= 0.5
        elif controller.zone == 'yellow':
          exploration_term *= 0.8

      tester_reward = (
          controller.lambda_task * norm_task
          + exploration_term
    ...
    tran['log/competence_zone'] = np.float32({
        'green': 2.0,
        'yellow': 1.0,
        'red': 0.0,
        'warmup': -1.0,
