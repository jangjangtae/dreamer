...
# ====================================================================
class RNDModule:
  def __init__(self, input_dim=16*16*3, hidden_dim=128, output_dim=128, lr=0.005):
    self.lr = lr
    self.output_dim = output_dim
    
    # 1. 고정된 타겟 네트워크 (학습 안 함, 랜덤 가중치)
    self.W1_t = np.random.randn(input_dim, hidden_dim) / np.sqrt(input_dim)
    self.b1_t = np.zeros(hidden_dim)
    self.W2_t = np.random.randn(hidden_dim, output_dim) / np.sqrt(hidden_dim)
    self.b2_t = np.zeros(output_dim)

    # 2. 타겟을 예측할 프레디터 네트워크 (학습 함)
    self.W1_p = np.random.randn(input_dim, hidden_dim) / np.sqrt(input_dim)
    self.b1_p = np.zeros(hidden_dim)
    self.W2_p = np.random.randn(hidden_dim, output_dim) / np.sqrt(hidden_dim)
    self.b2_p = np.zeros(output_dim)

  def compute_intrinsic_reward(self, obs):
    # 1. 속도를 위해 64x64 이미지를 16x16으로 축소 후 1차원 배열로 펼침
    small_obs = obs[::4, ::4, :] 
    x = small_obs.flatten().astype(np.float32) / 255.0

    # 2. Target 네트워크 통과 (Forward)
    z1_t = np.dot(x, self.W1_t) + self.b1_t
    h_t = np.maximum(0, z1_t) # ReLU
    y_t = np.dot(h_t, self.W2_t) + self.b2_t

    # 3. Predictor 네트워크 통과 (Forward)
    ...
    # 4. 내재적 보상 (MSE 오차 계산)
    diff = y_p - y_t
    reward = np.mean(diff ** 2)

    # 5. 프레디터 학습 (NumPy 수동 Backpropagation)
...
class Crafter(embodied.Env):
  ...
  def step(self, action):
    ...
    intrinsic_r = 0.0
    if self._use_rnd:
        intrinsic_r = self._rnd_mod.compute_intrinsic_reward(image)
    ...
    if self._done:
      ...
      if self._record_gifs and self._semantic_fault_episode_seen > 0:
  ...
  def _record_hud_frame(self, image, requested_action, info):
    try:
      ...
      img_pil = Image.fromarray(image).resize((512, 512), Image.NEAREST)
      draw = ImageDraw.Draw(img_pil)

      act_str = crafter.constants.actions[requested_action] if requested_action < len(crafter.constants.actions) else "unknown"
      fault_type = getattr(self, '_fault_spec', {}).get('type', 'None') if getattr(self, '_fault_spec', None) else "None"
      ...
      if sem_applied > 0:
        msg += "\n\n>>> SEMANTIC BUG TRIGGERED! <<<"
        draw.text((15, 15), msg, fill=(255, 50, 50))
      else:
      ...
      self._episode_frames.append(image)
