...
class Crafter(embodied.Env):
  ...
  def __init__(
    ...
    # family-aware reward scaling: progression-like contexts are common under
    # normal gameplay, while retry / reuse / delayed validation contexts are
    # more tester-specific and should receive larger bonuses.
    ...
    # Repeat penalty is intentionally conservative: semantic testing often needs
    # retry / revisit / delayed-followup behavior, so over-penalizing repeats
    # can suppress tester-like validation procedures.
  ...
  def _tester_reward_from_semantics(self, task_reward, requested_action, info):
    ...
    # Suspend repeat penalty during meaningful semantic validation:
    # - explicit semantic contexts (retry / relocate / delayed follow-up / etc.)
    # - fault-applied steps
    # - post-fault probing windows
    ...
    for key in context_keys:
      ...
      if not state['ctx']:
        ctx_reward = self._ctx_reward_for_key(key)
        state['ctx'] = 1
        bonus += ctx_reward
    ...
    if semantic_fault_applied and semantic_fault_type != 'none':
      ...
      state = self._semantic_proc_state[key]
      if not state['anom']:
        anom_reward = self._anom_reward_for_type(key)
        state['anom'] = 1
        bonus += anom_reward
