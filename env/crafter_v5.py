...
class Crafter(embodied.Env):
  ...
  def _write_episode_summary(self, length, reward, info):
    ...
    ctx_upgrade = int(info.get('semantic_upgrade_collect_count', 0))
    ctx_retry = int(info.get('semantic_retry_craft_count', 0))
    ctx_relocate = int(info.get('semantic_relocate_station_count', 0))
    ...
    summary = {
        ...
        'semantic_fault_count': int(info.get('semantic_fault_count', 0)),
        'semantic_trigger_reach': int(semantic_trigger_count > 0),
