...
class Crafter(embodied.Env):
  ...
  def __init__(
    ...
    forced_output_dir = os.getenv('CRAFTER_OUTPUT_DIR', '').strip()

    if forced_output_dir:
      self._output_dir = Path(forced_output_dir).expanduser().resolve()
    elif explicit_trace is not None:
      self._output_dir = explicit_trace.resolve().parent
    elif self._logdir:
      self._output_dir = Path(str(self._logdir)).expanduser().resolve()
    else:
      self._output_dir = Path('/home/railab/logdir').resolve()
