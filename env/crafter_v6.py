...
class Crafter(embodied.Env):
  ...
  def __init__(
    ...
    if trace_path is None:
      ...
      if env_trace:
        ...
        trace_path = str(self._logdir / f'fault_trace_{self._fault_profile}.jsonl')
      else:
        trace_path = '/home/railab/logdir/fault_trace.jsonl'
    ...
    # unified output directory for env-side files
    if self._logdir:
      self._output_dir = Path(str(self._logdir))
    elif self._trace_path is not None:
      self._output_dir = self._trace_path.parent
    else:
    ...
    if self._output_dir is not None:
      ...
      self._episode_summary_path = None
  ...
  def _write_episode_summary(self, length, reward, info):
    ...
    summary = {
        ...
        'fault_family': self._ep_fault_family,
        'fault_type': self._ep_fault_type,
        'fault_trigger': self._ep_fault_trigger,
        ...
        'semantic_fault_type': self._ep_fault_type if self._ep_fault_family == 'semantic_high_level' else info.get('semantic_fault_type', 'none'),
        ...
        'semantic_bug_family_coverage': int((ctx_upgrade > 0) + (ctx_retry > 0) + (ctx_relocate > 0)),
  ...
  def step(self, action):
    ...
    info['fault_episode'] = int(self._fault_episode or semantic_fault_episode or self._semantic_fault_episode_seen)
    ...
    if self._trace_path is not None:
      record = {
          ...
          'reward': float(reward),
          'fault_episode': int(info.get('fault_episode', self._fault_episode or semantic_fault_episode or self._semantic_fault_episode_seen)),
